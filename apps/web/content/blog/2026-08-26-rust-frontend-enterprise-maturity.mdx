...
**The performance premise inverts, which is worth knowing before the argument starts.** WebAssembly has no direct DOM access, so every DOM operation is a call back through JavaScript, and the deciding variable is not which runtime is faster but how often data crosses that boundary per operation. Frequent small crossings, which is exactly what per-node DOM manipulation is, erase the compiled-code advantage. Frameworks handle this by batching. Hand-written `web-sys` code crosses per call unless you build the batching yourself, and the framework you skipped had already done it: in `js-framework-benchmark`-style DOM tests, Leptos and Dioxus land competitive with SolidJS and ahead of React. So "skip the framework for speed" gets you a slower DOM path than the framework, while the genuine wasm win (CPU-bound work, roughly the 2–3× range on the right workload) is available either way from a Worker.
...
- **No conformance results published by any Rust UI component library.** `radix-leptos` claims WAI-ARIA compliance in its README and ships no test evidence, which is a claim rather than a result.
- **No reproducible `web-sys` DOM microbenchmark.** The boundary-crossing cost is well documented in principle, and per-call `setAttribute` overhead against a batched transfer is not published. The framework-level numbers in `js-framework-benchmark` are the closest available evidence.
- **No named engineering retrospective on migrating off a bespoke frontend framework.** The build-versus-buy cost models exist, and the first-person postmortem did not surface in this pass.
