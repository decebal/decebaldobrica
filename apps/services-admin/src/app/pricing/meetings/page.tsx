...
export default function MeetingsPricingPage() {
  const [meetings, setMeetings] = useState<MeetingPricing[]>([])
  ...
  async function loadMeetings() {
    try {
      const response = await fetch('/api/pricing/meetings')
  ...
  async function toggleActive(id: string, currentState: boolean) {
    try {
      await fetch(`/api/pricing/meetings/${id}`, {
        method: 'PATCH',
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="mb-8 flex items-center justify-between">
          <div>
            ...
            <p className="text-gray-600 dark:text-gray-400">
              Configure pricing for different meeting durations and types
            </p>
          </div>
          <button
            type="button"
            ...
            >
              <div className="flex items-start justify-between">
                  ...
                  <div className="grid grid-cols-2 gap-4 mb-4">
                    <div>
                      ...
                      <p className="text-lg font-bold text-gray-900 dark:text-white">
                        {meeting.price_sol?.toFixed(3) || '0.000'} SOL
                      </p>
                    </div>
                    <div>
                      <p className="text-sm text-gray-500 dark:text-gray-400">USD Price</p>
                      <p className="text-lg font-bold text-gray-900 dark:text-white">
                        ${meeting.price_usd?.toFixed(2) || '0.00'}
                      </p>
                    </div>
                  </div>

                  {/* Benefits */}
                  {meeting.benefits && meeting.benefits.length > 0 && (
                    <div>
                      <p className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-1">
                        Includes:
                      </p>
                      <ul className="space-y-1">
                        {meeting.benefits.map((benefit, idx) => (
                          <li
                            key={idx}
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => toggleActive(meeting.id, meeting.is_active)}
                  ...
                  <button
                    type="button"
                    onClick={() => togglePopular(meeting.id, meeting.is_popular)}
                  ...
                  >
                    {meeting.is_popular ? '⭐ Unmark' : 'Mark Popular'}
                  </button>
                </div>
              </div>
            </div>
        ...
        {meetings.length === 0 && (
          <div className="text-center py-12">
            <p className="text-gray-500 dark:text-gray-400 mb-4">No meeting types found</p>
            <button
              type="button"
